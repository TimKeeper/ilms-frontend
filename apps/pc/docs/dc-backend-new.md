...
原始数据预览接口，且仅能够浏览最多100页。因数据量巨大，无法使用传统分页手段。首次查询必定是第一页，且会返回下一页的游标cursor。用户交互时，只能点击上一页和下一页，前端需要记录当前筛选条件下和页大小的游标信息，方便点击上一页时可以直接找到对应的游标。若用户修改了筛选条件以及页大小，则重新从第一页开始。注意：1、前端限制最多翻页100页，再多影响性能。2、后端接口若没返回cursor，说明肯定不存在下一页。3、后端接口若返回了cursor，不一定说明肯定存在下一页，若传入cursor后继续查询返回的为空，则提示用户已经是最后一页。
...
只支持.xls或.xlsx文件导入接口会对文件内的数据进行预检查。只有检测全部通过才会进行导入。只要有数据检测不通过，就不会进行导入，接口会返回所有有问题的数据，供用户修改参考。
...
只支持.xls或.xlsx文件导入接口会对文件内的数据进行预检查。检测通过的会先行导入。数据检测不通过，就不会进行导入，接口会返回所有有问题的数据，供用户修改参考。
...
```json
{
  ...
  "data": {
    ...
    "successCount": 0,
    "failCount": 1,
    "errors": ["第2行: 已存在相同高温标识器ID【1002】，被【铁包1】铁包绑定"]
  }
}
```
...
该串读列表记录的是同一个标签在相同的时间段内被2个工位雷达同时扫描到。会反馈两个工位雷达扫描到的时间节点，以及标签本身信息。
...
```json
{
  ...
  "resMsg": "成功",
  "data": {
    "xaxis": ["2025-12-27", "2025-12-28", "2025-12-29", "2025-12-30"],
    "series": [
      {
        "tagBoundName": "车架1号",
        "data": [367, 367, 366, 367],
        "tagSn": 5283351,
...
当用户点击导出雷达原始数据，或雷达数据组时，会生成一条导出记录表，系统将在后台执行导出工作。当后台执行完成后，刷新列表即可看到导出状态，当状态为成功时，代表可以下载，执行下载导出文件接口即可。
